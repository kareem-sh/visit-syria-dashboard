...
const DocumentViewer = ({ documents, initialIndex = 0, onClose }) => {
    ...
    const isImage = currentDoc.type?.startsWith("image/") ||
        (currentDoc.name && /\.(jpg|jpeg|png|gif|bmp|webp)$/i.test(currentDoc.name)) ||
        (currentDoc.url && /\.(jpg|jpeg|png|gif|bmp|webp)$/i.test(currentDoc.url));
